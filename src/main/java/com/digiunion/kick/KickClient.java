...
import com.digiunion.kick.model.kivarino.ChannelData;
import com.digiunion.kick.model.UserData;
import com.digiunion.kick.model.MessageData;
...
import com.digiunion.kick.util.KickEndpoints;
import com.digiunion.kick.util.KickOfficialAPIEndpoints;
...
public class KickClient {
  ...
  public KickClient(){
    ...
    try {
      ...
      executor1 = Executors.newVirtualThreadPerTaskExecutor();
  ...
  //
  //
  public CompletableFuture<WebSocket> connect(Listener webSocketListener) {
    return httpClient.newWebSocketBuilder().buildAsync(URI.create(KivarinoURLs.EVENTS.url), webSocketListener);
//.sendAsync(java.net.http.HttpRequest.newBuilder(URI.create(KivarinoURLs.EVENTS.url)).build(), BodyHandlers.ofString());
  }
  public CompletableFuture<UserData> getUserByToken(String token){
    if(token != null)
    return httpClient.sendAsync(java.net.http.HttpRequest.newBuilder(URI.create(KickOfficialAPIEndpoints.USERS.url)).GET().header("Authorization", new StringBuilder("Bearer ").append(token).toString()).build(), BodyHandlers.ofString())
    .thenApply(response -> {
      //System.out.println(response);
      try {
        return mapper.readValue(response.body(), UserData.class);
      }catch (JsonProcessingException e) {
        System.err.printf("[\033[31mSEVERE\033[0m] could not introspect token; %s\n", e.getMessage());
        return null;
      }
    });
    else 
  ...
  public CompletableFuture<UserData> getChannelById(char[] token, String id){
    if(id != null)
    return httpClient.sendAsync(java.net.http.HttpRequest.newBuilder(URI.create(KickOfficialAPIEndpoints.CHANNELS.url + "?id=" + id)).GET().header("Authorization", new StringBuilder("Bearer ").append(token).toString()).build(), BodyHandlers.ofString())
    .thenApply(response -> {
      //System.out.println(response);
      try {
        return mapper.readValue(response.body(), UserData.class);
  ...
  public CompletableFuture<Credentials> refresh(Credentials token) {

    if(token != null && !token.access_token().isBlank())
    return httpClient.sendAsync(java.net.http.HttpRequest.newBuilder(URI.create(new StringBuilder(KivarinoURLs.OAUTH.url).append("?refresh_token=").append(URLEncoder.encode(token.refresh_token(), StandardCharsets.UTF_8)).toString())).header("Authorization", token.access_token()).POST(BodyPublishers.noBody()).build(), BodyHandlers.ofString()).thenApply(res -> {
      try {
        //System.out.printf("body: %s\n", res.body());
        return mapper.readValue(res.body(), Credentials.class);
      } catch (JsonProcessingException e) {
        System.err.printf("[\033[31mSEVERE\033[0m] could not parse token; %s\n", e.getMessage());
        return null;
      }
    });
  ...
  public CompletableFuture<ChannelApi> getChannelBySlug(Credentials creds, String channelSlug) {
    if(channelSlug != null)
    return httpClient.sendAsync(java.net.http.HttpRequest.newBuilder(URI.create(new StringBuilder(KickOfficialAPIEndpoints.CHANNELS.url).append("?slug=").append(channelSlug.replace(' ', '\0')).toString())).header("Authorization", "Bearer " + creds.access_token()).GET().build(), BodyHandlers.ofString()).thenApply(res -> {

      try {

        //System.out.printf("[\033[34mINFO\033[0m] response body %s, status %d\n", res.body(), res.statusCode());
        return mapper.readValue(res.body(), ChannelData.class).data()[0];
      } catch (JsonProcessingException e) {
        System.err.printf("[\033[31mSEVERE\033[0m] could not parse token; %s\n", e.getMessage());
        return null;
      }
    ...
    public CompletableFuture<MessageData> postMessage(Credentials creds, long channelId, String content) {
    if(channelId != 0)
    return httpClient.sendAsync(java.net.http.HttpRequest.newBuilder(URI.create(KickOfficialAPIEndpoints.CHAT.url)).headers("Authorization", "Bearer " + creds.access_token(), "Content-Type", "application/json").POST(BodyPublishers.ofString(String.format("{\"broadcaster_user_id\": %d,\"content\": \"%s\", \"type\": \"user\"}", channelId, content))).build(), BodyHandlers.ofString()).thenApply(res -> {
      try {

        System.out.printf("[\033[34mINFO\033[0m] response body %s, status %d\n", res.body(), res.statusCode());
        return mapper.readValue(res.body(), MessageData.class);
      } catch (JsonProcessingException e) {
        System.err.printf("[\033[31mSEVERE\033[0m] could not parse token; %s\n", e.getMessage());
        return null;
      }
...
public CompletableFuture<ChannelApi[]> getChannelsBySlug(Credentials creds, String[] channelSlug) {
    if(channelSlug != null){
      var sb = new StringBuilder(KickOfficialAPIEndpoints.CHANNELS.url).append("?slug=");
      for(int i = 0; i < channelSlug.length-1; i++) {
        sb.append(channelSlug[i]).append("&slug=");
      }
      sb.append(channelSlug[channelSlug.length-1]);
    return httpClient.sendAsync(java.net.http.HttpRequest.newBuilder(URI.create(sb.toString())).header("Authorization", "Bearer " + creds.access_token()).GET().build(), BodyHandlers.ofString()).thenApply(res -> {

      try {

        //System.out.printf("[\033[34mINFO\033[0m] response body %s, status %d\n", res.body(), res.statusCode());
        return mapper.readValue(res.body(), ChannelData.class).data();
      } catch (JsonProcessingException e) {
        System.err.printf("[\033[31mSEVERE\033[0m] could not parse token; %s\n", e.getMessage());
        return null;
      }
  ...
  public CompletableFuture<TokenIntrospection> introspect(String accessToken) {
    if(accessToken != null)
    return httpClient.sendAsync(java.net.http.HttpRequest.newBuilder(URI.create(KickOfficialAPIEndpoints.TOKEN_INTROSPECTION.url)).header("Authorization", "Bearer " + accessToken).POST(BodyPublishers.noBody()).build(), BodyHandlers.ofString()).thenApply(res -> {

      try {

        //System.out.printf("[\033[34mINFO\033[0m] response body %s, status %d\n", res.body(), res.statusCode());
        return mapper.readValue(res.body(), TokenIntrospection.class);
      } catch (JsonProcessingException e) {
        System.err.printf("[\033[31mSEVERE\033[0m] could not parse token; %s\n", e.getMessage());
        return null;
      }
  ...
  public CompletableFuture<Channel> getChannel(String slug){
    return CompletableFuture.supplyAsync(() -> {
      client.set(rClient);
      try (final Response response = client.get().newCall(new Request.Builder().url(KickEndpoints.CHANNELS.url.concat(slug)).get().build()).execute()) {
  ...
  public CompletableFuture<Livestream> getLivestreamJ(String slug){
    final CompletableFuture<Livestream> resultt = eventloop.submit(()->
      aCleint.request(HttpRequest.get(KickEndpoints.CHANNELS.url.concat(slug))
