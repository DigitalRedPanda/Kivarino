...
public class Tab extends Region {
  ...
  public Tab(String channelName) {
    ...
    setOnMousePressed(clickEvent -> {
      if (Tab.focusedTab != this || isActive) {
        if(Tab.focusedTab != null) {
          deactivateTab(Tab.focusedTab);
        }
        activateTab(this);
        isActive = true;
      } else {
  ...
  private void activateTab(Tab tab) {
    Tab.focusedTab = this;
    GUI.borderPane.setCenter(GUI.channelsChats.get(this.text.getText()));
    tab.setStyle("-fx-background-color: #0B6623;");
    tab.colorAdjust.setBrightness(0.25);
    tab.closeText.setVisible(true);
    tab.closeText.setFont(Font.font(15));
    tab.hBox.setPadding(hoveredInset);
  }

private void deactivateTab(Tab tab) {
    tab.setStyle("-fx-background-color: #404446;");
    tab.closeText.setVisible(false);
    tab.colorAdjust.setBrightness(0);
    tab.hBox.setPadding(unhoveredInset);
    tab.closeText.setFont(Font.font(0));
}
